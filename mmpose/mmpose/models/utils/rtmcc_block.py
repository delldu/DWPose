...
class RTMCCBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if in_token_dims == out_token_dims: # True
            self.shortcut = True
    ...
    def _forward(self, inputs):
        ...
        if self.attn_type == 'self-attn': # True
            x = inputs
        else:
            x, k, v = inputs
    ...
    def forward(self, x):
        ...
        if self.shortcut: # True
            if self.attn_type == 'cross-attn': # False
                res_shortcut = x[0]
            else:
                res_shortcut = x
